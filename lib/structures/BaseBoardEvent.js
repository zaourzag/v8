...
module.exports = class extends Event {
	...
	buildEmbed(msg, guild, channel, message, reacts) {
		...
		const embed = new MessageEmbed()
			.setAuthor({ name: msg.author.username, iconURL: msg.author.displayAvatarURL({ format: 'png' }) })
		...
		if (msg.attachments?.size) {
			const [_images, _files] = msg.attachments.partition(attachment => attachment.height);
			if (_images.size) embed.setImage(_images.first().url);
			if (_files.size) files = [..._files.values()];
		}
		return { embed, files };
	}
