...
Structures.extend('Guild', Guild => {
	class AeroGuild extends Guild {
		...
		async createMuteRole(name) {
			...
			for (const channel of this.channels.cache.filter(chan => chan.type === 'GUILD_TEXT').values())
		...
		async createRaidMessage() {
			...
			const userFilter = (user) =>
				!user.bot
				&& this.members.fetch(user.id).then(member => member.permissions.has(Permissions.FLAGS.BAN_MEMBERS));
