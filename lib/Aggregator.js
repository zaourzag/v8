...
class Aggregator {
		...
		cluster.on('message', (worker, msg) => {
			switch (msg?.type) {
				...
				case 'COMMAND':
					...
					break;
				case 'SCAM':
					this.registerScamDomain(msg.data.domain);
					break;
				case 'READY':
					if (Array.isArray(msg.data.shardId))
						msg.data.shardId.forEach(id => this.readyShards.add(id));
					else
	...
	get ready() {
		const isReady = this.readyShards.size === this.shardCount;
		if (!isReady) logger.log(`[Aggregator] Requested ready but only ${this.readyShards.size}/${this.shardCount} shards ready: ${[...this.readyShards].join(', ')}`);
		return isReady;
	}
	...
	initPrometheus() {
		...
		const scamDomainCount = new prometheus.Counter({
			name: 'aero_scams',
			help: 'global scams caught',
			labelNames: ['domain']
		});
		...
		register.registerMetric(scamDomainCount);

		this.prometheus = prometheus;
		this.commandCounter = commandCount;
		this.gbanCounter = globalBanCount;
		this.scamCounter = scamDomainCount;
