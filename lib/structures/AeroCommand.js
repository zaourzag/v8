...
class AeroCommand extends Command {
	...
	determineCategory() {
		// The file path will be like: commands/General/ping.js or commands/Utility/echo.js
		const pathParts = this.location.full.split('/');
		const commandsIndex = pathParts.findIndex(part => part === 'commands');
		if (commandsIndex !== -1 && pathParts.length > commandsIndex + 1) {
