const { Command } = require('@aero/framework'); const { Permissions: { FLAGS } } = require('discord.js');
module.exports = class extends Command {
	...
	constructor(...args) {
		super(...args, {
			...
			aliases: ['mr'], description: language => language.get('COMMAND_MUTEROLE_DESCRIPTION'),
			usage: '[rolename:rolename|roleid:role]'
		});
		this.defaultPermissions = FLAGS.MANAGE_ROLES;
	}
	async run(msg, [role]) {
		if (!role) return this.display(msg); await msg.guild.settings.sync(); await msg.guild.settings.update('mod.roles.mute', role.id);
