...
class Permissions {
		...
		switch (action) {
			...
			case 'deny': {
				...
				if (treeName === 'everyone') {
					...
					else makeObject(`everyone.${category}.${command}`, value, nodes);
				} else {
					/* eslint-disable no-lonely-if */
					if (!command) makeObject(`${treeName}.${target.id}.${category}`, value, nodes);
					else makeObject(`${treeName}.${target.id}.${category}.${command}`, value, nodes);
	...
	async canUse(msg, command) {
		...
		const roles = [...msg.member.roles.cache.sort((a, b) => b.position - a.position).values()].slice(0, -1);
	...
	validate(permission) {
		...
		const commands = this.client.stores.get('commands');
		if (command !== '*' && !commands.has(command)) throw 'COMMAND_PERMS_ERR_USAGE_INVALIDCOMMAND';
		return { category, command: commands.get(command) || command };
	}
	...
	buildTree(target, nodes, treeName, guild) {
		...
		switch (treeName) {
			case 'users':
				mergeObjects(tree, nodes.users[target.id]);
				Permissions.makeEntries(tree, guild, DIRECT, target.id);
			case 'roles':
				if (treeName === 'roles') {
					mergeObjects(tree, nodes.roles[target.id]);
			...
			case 'everyone':
				if (treeName === 'everyone') {
					mergeObjects(tree, nodes.everyone);
					Permissions.makeEntries(tree, guild, DIRECT);
				} else {
	...
	mergePermissions(target, source) {
		...
		for (const key in target) {
			...
			target[key] = mergeObjects(source[key], target[key]);
		}
		return target;
	}
	...
	init() {
		const commands = this.client.stores.get('commands');
		this.categories = new Set([...commands.values()].map(command => command.category.toLowerCase()));
	}
